...
class SendThankYouHandler extends BaseActionHandler
{
    protected function handleAction(Contact $contact, array $parameters): array
    {
        // Get the template from parameters
        $templateName = $parameters['template'] ?? 'Thank You for Your Donation';
        $delay = $parameters['delay'] ?? 0;

        \Log::info("SendThankYouHandler: Handling action", [
            'contact' => $contact->id,
        ...
        if (!$template) {
            \Log::error("SendThankYouHandler: Template not found", ['template_name' => $templateName]);
            throw new \RuntimeException("Template not found: {$templateName}");
        }

        \Log::info("SendThankYouHandler: Found template", ['template_id' => $template->id]);
        ...
        \Log::info("SendThankYouHandler: Created touch", ['touch_id' => $touch->id]);
        ...
        try {
            ...
            \Log::info("SendThankYouHandler: Email sent and touch updated", ['touch_id' => $touch->id]);
            ...
            \Log::error("SendThankYouHandler: Failed to send email", [
                'touch_id' => $touch->id,
                'error' => $e->getMessage()
            ]);
